...
def load_employee_history(name, df, dt, client):
    ...
    if df:  q = q.gte("week_date", str(df))
    if dt:  q = q.lte("week_date", str(dt))
...
c1.metric("Total Hours",  f"{total_hours:.1f} hrs")
...
st.subheader(f"📋 Weekly History — {len(df_hist)} record(s)")

df_show = df_hist[["week_date","client_name","hours_worked",
                   "payroll_amount","self_emp_amount","utr_amount","total_paid","notes"]].copy()
df_show["week_date"] = pd.to_datetime(df_show["week_date"]).dt.strftime("%d %b %Y")
df_show.columns = ["Week Start","Hotel","Hours","Payroll £","Self-Emp £","UTR £","Total Paid £","Notes"]
...
totals_row = pd.DataFrame([{
    ...
    "Self-Emp £":  round(total_self, 2),
    "UTR £":       round(total_utr, 2),
...
# ── Add / edit notes ──────────────────────────────────────────
st.markdown("---")
st.subheader("📝 Add Note to a Week")
if len(df_hist) > 0:
    week_options = []
    for _, r in df_hist.iterrows():
        try:
            label = datetime.strptime(str(r["week_date"]), "%Y-%m-%d").strftime("w/c %d %b %Y")
        except:
            label = str(r["week_date"])
        week_options.append((label, r["id"], r.get("notes", "") or ""))

    note_labels = [w[0] for w in week_options]
    selected_week_idx = st.selectbox("Select week", range(len(note_labels)),
                                     format_func=lambda i: note_labels[i])

    selected_id    = week_options[selected_week_idx][1]
    existing_note  = week_options[selected_week_idx][2]
    new_note = st.text_area("Note", value=existing_note, height=80,
                             placeholder="e.g. Bank holiday deduction, absence, bonus note…")
