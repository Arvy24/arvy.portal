...
def load_week_records(wd):
    return supabase.table("weekly_records").select(
        "id,employee_name,client_name,hours_worked,utr_amount"
        ...
        if rows:
            ...
            if st.button("💾 Save UTR Payments", type="primary", use_container_width=True):
                ...
                if errors:
                    ...
                    st.success(f"✅ UTR payments saved for {saved} employees — Week **{week_date.strftime('%d %b %Y')}**")
                    st.balloons()
        else:
            st.warning("No valid rows found. Check column mapping.")

st.markdown("---")
st.subheader(f"📋 UTR Records — {week_date.strftime('%d %b %Y')}")
try:
